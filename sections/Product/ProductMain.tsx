export default function ProductMain() {

    return (
        <div className="container flex flex-col lg:flex-row gap-4 my-6 px-5 lg:p-0">
            {/* Galeria do produto */}
            <div className="flex-1 bg-slate-400 h-96">
                teste
            </div>
            <div className="flex flex-col gap-2 flex-1">
                <span className="text-sm text=[#848585] uppercase font-light">ERFWV3HUS</span>
                <h2 className="text-xl text-[#323333]">Nevecón Electrolux French Door No Frost 298Lts ERFWV3HUS</h2>
                <button className="flex justify-center items-center w-full lg:max-w-80 h-[42px] rounded-3xl bg-[#EE405A] text-white text-sm font-semibold mt-4">
                    Where to buy
                </button>
            </div>
        </div>
    );
}